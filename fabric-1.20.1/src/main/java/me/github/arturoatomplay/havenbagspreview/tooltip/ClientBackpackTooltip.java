...
public class ClientBackpackTooltip implements ClientTooltipComponent {
    ...
    public void renderImage(Font font, int mouseX, int mouseY, GuiGraphics guiGraphics) {
        ...
        for (int l = 0; l < j; ++l) {
            for (int m = 0; m < i; ++m) {
                ...
                this.renderSlot(n, o, k++, font, guiGraphics);
            }
        }

        this.drawBorder(mouseX, mouseY, i, j, guiGraphics);
    }

    private void renderSlot(int x, int y, int itemIndex, Font font, GuiGraphics guiGraphics) {
    ...
    private void drawBorder(int x, int y, int slotWidth, int slotHeight, GuiGraphics guiGraphics) {
        ...
        this.blit(guiGraphics, x + slotWidth * SLOT_SIZE_X + 1, y, Texture.BORDER_CORNER_TOP);

        for (int i = 0; i < slotWidth; ++i) {
            this.blit(guiGraphics, x + 1 + i * SLOT_SIZE_X, y, Texture.BORDER_HORIZONTAL_TOP);
            this.blit(guiGraphics, x + 1 + i * SLOT_SIZE_X, y + slotHeight * SLOT_SIZE_Y + 1, Texture.BORDER_HORIZONTAL_BOTTOM);
        }

        for (int i = 0; i < slotHeight; ++i) {
            this.blit(guiGraphics, x, y + i * SLOT_SIZE_Y + 1, Texture.BORDER_VERTICAL);
            this.blit(guiGraphics, x + slotWidth * SLOT_SIZE_X + 1, y + i * SLOT_SIZE_Y + 1, Texture.BORDER_VERTICAL);
        }

        this.blit(guiGraphics, x, y + slotHeight * SLOT_SIZE_Y + 1, Texture.BORDER_CORNER_BOTTOM);
        this.blit(guiGraphics, x + slotWidth * SLOT_SIZE_X + 1, y + slotHeight * SLOT_SIZE_Y + 1, Texture.BORDER_CORNER_BOTTOM);
    }

    private void blit(GuiGraphics guiGraphics, int x, int y, Texture texture) {
        guiGraphics.blit(TEXTURE_LOCATION, x, y, (float)texture.x, (float)texture.y, texture.w, texture.h, 128, 128);
    }
